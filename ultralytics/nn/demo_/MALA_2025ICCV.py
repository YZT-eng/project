...
__all__ =['MALAttention','C2f_MALA','C3k2_MALA','C2f_MALABlock','C3k2_MALABlock',
          'DSC3k2_MALA','DSC3k2_MALABlock']
...
class MALABlock(nn.Module):
    ...
    def __init__(self,  embed_dim, num_heads, ffn_dim, flag='l',drop_path=0., layerscale=False, layer_init_value=1e-6):
        super().__init__()
        ...
        assert flag in ['l', 'v']
        if flag == 'l':
            self.attn = MALAttention(embed_dim, num_heads)
        else:
...
class Bottleneck_MALA(nn.Module):
    """Standard bottleneck."""
    ...
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        ...
        self.Attention = MALAttention(c2,8)

    def forward(self, x):
        """'forward()' applies the YOLO FPN to input data."""
        return x + self.Attention(self.cv2(self.cv1(x))) if self.add else self.Attention(self.cv2(self.cv1(x)))

class C2f_MALA(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.m = nn.ModuleList(Bottleneck_MALA(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
...
class C2f_MALABlock(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv1 = Conv(c1, 2 * self.c, 1, 1)
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(MALABlock(self.c,8,self.c) for _ in range(n))
...
class C3k(C3):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(Bottleneck_MALA(c_, c_, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n)))
class C3k_MALABlock(C3):
    """C3k is a CSP bottleneck module with customizable kernel sizes for feature extraction in neural networks."""
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5, k=3):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(MALABlock(self.c,8,self.c)  for _ in range(n)))
class C3k2_MALA(C2f_MALA):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_MALA(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n)
        )
class C3k2_MALABlock(C2f_MALA):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    ...
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        ...
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else MALABlock(self.c,8,self.c)  for _ in range(n)
        )
#YOLOV13改进
...
class DSBottleneck_MALA(nn.Module):
    """
...
class DSC3k_MALABlock(C3):
    ...
    def __init__( self,c1,c2,n=1,shortcut=True,g=1,e=0.5,k1=3,k2=5,d2=1):
        ...
        self.m = nn.Sequential(
            *(
                MALABlock(self.c,8,self.c)
                for _ in range(n)
            )
        )
class DSC3k2_MALA(C2f_MALA):
    """
    ...
    def __init__(self,c1,c2,n=1,dsc3k=False,e=0.5,g=1,shortcut=True,k1=3,k2=7,d2=1):
        super().__init__(c1, c2, n, shortcut, g, e)
        if dsc3k:
            self.m = nn.ModuleList(DSC3k_MALA(self.c, self.c,n=2,shortcut=shortcut,g=g,e=1.0,k1=k1,k2=k2,d2=d2)for _ in range(n))
        else:
            self.m = nn.ModuleList(DSBottleneck_MALA(self.c, self.c,shortcut=shortcut,e=1.0,k1=k1,k2=k2,d2=d2)for _ in range(n))

class DSC3k2_MALABlock(C2f_MALA):
    """
    An improved C3k2 module that uses lightweight depthwise separable convolution blocks.
    ...
    def __init__(self,c1,c2,n=1,dsc3k=False,e=0.5,g=1,shortcut=True,k1=3,k2=7,d2=1):
        super().__init__(c1, c2, n, shortcut, g, e)
        if dsc3k:
            self.m = nn.ModuleList(DSC3k_MALABlock(self.c, self.c,n=2,shortcut=shortcut,g=g,e=1.0,k1=k1,k2=k2,d2=d2) for _ in range(n))
        else:
            self.m = nn.ModuleList(MALABlock(self.c,8,self.c) for _ in range(n))
